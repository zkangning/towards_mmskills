...
| --- | ---: | ---: | ---: | --- | --- |
| `no_skills` | 0.0 | 20 | 98.0s + 50.0s | [no_skills_1080p.mp4](no_skills_1080p.mp4) | No-skills: repeatedly clicks around the sheet tab/context-menu area and never constructs the requested merged-cell layout. |
| `text_only` | 0.0 | 20 | 120.0s + 50.0s | [text_only_1080p.mp4](text_only_1080p.mp4) | Text-only: starts toward the right operation, but loses cell selection state and ends by rewriting only Investment Summary, without the two second-row headers. |
| `multimodal_v9` | 1.0 | 12 | 80.0s + 50.0s | [multimodal_1080p.mp4](multimodal_1080p.mp4) | Multimodal v9: creates Sheet2, selects each target range, merges A1:C1, A2:B2, and C2:D2, then writes all three labels. |
